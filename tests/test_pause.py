...
def policy(**kw):
    v={"allowed_severities":["CRITICAL","HIGH"],"max_pause_seconds":86400,"min_pause_seconds":900,"policy_revision":1,"protocol_code":"BRIDGE_ALPHA"};v.update(kw);return json.dumps(v,sort_keys=True)
def incident(**kw):
    v={"affected_contract":AFFECTED,"capability":"BRIDGE_WITHDRAWALS","chain_id":8453,"incident_summary":"An active withdrawal proof bypass permits immediate unauthorized release of escrowed user assets and remains unpatched.","incident_tx_hash":TX,"observed_at":"2026-09-06T00:00:00Z","protocol_code":"BRIDGE_ALPHA","severity":"CRITICAL"};v.update(kw);return json.dumps(v,sort_keys=True)
def git_blob(body): return hashlib.sha1(b"blob "+str(len(body)).encode()+b"\x00"+body).hexdigest()
def mock_sources(vm, inc=None, pol=None, status=200, commit=COMMIT, truncated=False, corrupt=False, tx_status=200, tx_hash=TX, tx_to=AFFECTED, tx_result="ok"):
    ...
    vm.mock_web(r"base\.blockscout\.com/api/v2/transactions/",{"status":tx_status,"body":json.dumps({"hash":tx_hash,"to":{"hash":tx_to},"status":tx_result})})
...
def prepared(d,vm):
    c=deploy(d);setup(c);request(c,vm);return c
...
def test_justified_consume_scope_and_replay(direct_deploy,direct_vm):
    ...
    assert c.assess_pause(1,1)=="PAUSE_JUSTIFIED";r=c.get_case(1);assert r["revision"]==2 and r["evidence_digest"].startswith("sha256:")
...
def test_unresolved_revision_recovery(direct_deploy,direct_vm):
    ...
    direct_vm.clear_mocks();mock_sources(direct_vm);mock_model(direct_vm);assert c.assess_pause(1,2)=="PAUSE_JUSTIFIED";assert c.get_case(1)["revision"]==3
...
def test_expired_authorization(direct_deploy,direct_vm):
    direct_vm.warp("2026-09-06T00:00:00Z");c=prepared(direct_deploy,direct_vm);mock_sources(direct_vm);mock_model(direct_vm);c.assess_pause(1,1)
